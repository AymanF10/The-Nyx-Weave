...
#[derive(Accounts)]
#[instruction(token_to_deposit: Pubkey, risk_level: u8)]
pub struct UserDeposit<'info> {
    ...
    #[account(
        ...
        seeds = [b"depositor", depositor.key().as_ref(), token_to_deposit.key().as_ref()],
        bump,
    )]
...
/// Process a user deposit into a strategy vault
/// Transfers tokens from user to strategy and updates balances
pub fn create_deposit(ctx: Context<UserDeposit>, amount: u64) -> Result<()> {
...
/// Process a user withdrawal from a strategy vault
/// Transfers tokens from strategy to user and updates balances
pub fn withdraw(ctx: Context<UserDeposit>, amount: u64) -> Result<()> {
    let strategy_vault_info = &mut ctx.accounts.strategy_vault;
    let depositor_account_info = &mut ctx.accounts.depositor_account;
    ...
    let vault_authority_seeds = &[
        b"strategy_vault",
        strategy_vault_info.to_account_info().key.as_ref(),
        &strategy_vault_info.risk_level.to_be_bytes(),
