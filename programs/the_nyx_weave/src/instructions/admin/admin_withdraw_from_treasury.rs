...
pub struct WithdrawFromTreasury<'info> {
    ...
    #[account(mut, token::mint = jitosol_mint, token::authority = admin)]
    pub admin_jitosol_ata: Account<'info, TokenAccount>,
...
impl<'info> WithdrawFromTreasury<'info> {
    ...
    pub fn withdraw_all_tokens_from_treasury(&self) -> Result<()> {
        ...
        self.withdraw_token_from_treasury(self.jitosol_mint.key(), self.treasury_vault_jito_ata.amount)
    }
    ...
    fn select_accounts(
        ...
        } else if mint == self.jitosol_mint.key() {
            Ok((&self.treasury_vault_jito_ata, &self.admin_jitosol_ata, &self.jitosol_mint))
        } else {
